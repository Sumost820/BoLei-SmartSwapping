...
def selectDestroyedVehicles(solution, rng, iteration, for_gurobi):
    ...
    if method == 0 or not solution["schedule"]:
        return rng.sample(vehicles, size)
    ...
    if method == 2:
        ...
        return rng.sample(ranked[:poolSize], size)
...
def runGurobiLnsIteration(solution, destroyed, patterns_by_trips, upper_bound, rng):
    ...
    model.addConstr(trip_expression <= upper_bound["trip_upper_bound"] - fixed_trips, name="global_trip_upper_bound")
...
def runGurobiLns(initial_solution, patterns_by_trips, upper_bound, rng):
    ...
    for iteration in range(GUROBI_LNS_ITERATIONS):
        ...
        if totalTrips(best) >= upper_bound["trip_upper_bound"]:
            print("达到理论上界，提前结束")
            break
...
def main():
    ...
    if RUN_GUROBI_LNS:
        if totalTrips(best_solution) < upperBound["trip_upper_bound"]:
            print("\n================ 执行Gurobi大邻域搜索 ================")
